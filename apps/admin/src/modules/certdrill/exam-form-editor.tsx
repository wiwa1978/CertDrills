...
function MetadataCard({ certificationId, examForm }: { certificationId: string; examForm: CertDrillAdminExamForm }) {
  const [state, action, pending] = useActionState(updateCertDrillExamFormMetadataAction, initialExamFormActionState);
  return <Card><CardHeader><CardTitle>Details</CardTitle><CardDescription>Name and duration changes preserve the assignment.</CardDescription></CardHeader><CardContent><form action={action} className="grid gap-4 sm:grid-cols-2"><Hidden certificationId={certificationId} examForm={examForm} /><div className="space-y-2"><Label htmlFor="form-name">Name</Label><Input id="form-name" name="name" defaultValue={examForm.name} required /></div><div className="space-y-2"><Label htmlFor="form-duration">Duration minutes</Label><Input id="form-duration" name="durationMinutes" type="number" min="1" defaultValue={examForm.durationMinutes} required /></div>{state.formError ? <p role="alert" className="text-sm text-destructive sm:col-span-2">{state.formError}</p> : null}<Button className="sm:col-span-2 sm:w-fit" disabled={pending}>Save details</Button></form></CardContent></Card>;
}

function AssignmentActions({ certificationId, examForm }: { certificationId: string; examForm: CertDrillAdminExamForm }) {
...
function QuestionTable({ questions, categoriesById, alternatives, certificationId, examForm }: { questions: CertDrillAdminQuestion[]; categoriesById: Map<string, CertDrillAdminCategory>; alternatives: CertDrillAdminQuestion[]; certificationId: string; examForm: CertDrillAdminExamForm }) {
  return <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Question</TableHead><TableHead>Direct category</TableHead><TableHead>Difficulty</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{questions.map((question) => <TableRow key={question.id}><TableCell className="max-w-xl whitespace-normal">{question.stem}</TableCell><TableCell>{categoriesById.get(question.categoryId)?.name ?? question.categoryId}</TableCell><TableCell>{question.difficulty ?? "medium"}</TableCell><TableCell><Badge variant="outline">{question.status}</Badge></TableCell><TableCell className="text-right"><ReplaceDialog current={question} alternatives={alternatives} certificationId={certificationId} examForm={examForm} /></TableCell></TableRow>)}</TableBody></Table></div>;
}

function ReplaceDialog({ current, alternatives, certificationId, examForm }: { current: CertDrillAdminQuestion; alternatives: CertDrillAdminQuestion[]; certificationId: string; examForm: CertDrillAdminExamForm }) {
  const [state, action, pending] = useActionState(replaceCertDrillExamFormQuestionAction, initialExamFormActionState);
  if (!alternatives.length) return <div className="inline-flex flex-col items-end gap-1"><Button size="sm" variant="outline" disabled>Replace</Button><span className="max-w-48 text-xs text-muted-foreground">No eligible replacement is available.</span></div>;
  return <Dialog><DialogTrigger asChild><Button size="sm" variant="outline">Replace</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Replace question</DialogTitle><DialogDescription>Choose a published, unassigned question from the same top-level category.</DialogDescription></DialogHeader><form action={action} className="space-y-4"><Hidden certificationId={certificationId} examForm={examForm} version /><input type="hidden" name="currentQuestionId" value={current.id} /><div className="space-y-2"><Label htmlFor={`replacement-${current.id}`}>Replacement</Label><select id={`replacement-${current.id}`} name="replacementQuestionId" className="w-full rounded-md border bg-background p-2" required>{alternatives.map((question) => <option key={question.id} value={question.id}>{question.stem}</option>)}</select></div>{state.formError ? <p className="text-sm text-destructive">{state.formError}</p> : null}<Button disabled={pending}>Replace</Button></form></DialogContent></Dialog>;
}

function Hidden({ certificationId, examForm, version = false }: { certificationId: string; examForm: CertDrillAdminExamForm; version?: boolean }) { return <><input type="hidden" name="certificationId" value={certificationId} /><input type="hidden" name="examFormId" value={examForm.id} />{version ? <input type="hidden" name="expectedAssignmentVersion" value={examForm.assignmentVersion} /> : null}</>; }
