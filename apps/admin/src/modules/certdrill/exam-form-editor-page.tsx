import { listCertDrillAdminCategoriesServer, listCertDrillAdminCertificationsServer, listCertDrillAdminQuestionsServer, listCertDrillAdminScenariosServer, getCertDrillAdminExamFormServer } from "@/lib/api/certdrill.server";
...
export async function CertDrillExamFormEditorPage({ certificationId, examFormId }: { certificationId: string; examFormId: string }) {
  const [certifications, categories, questions, scenarios, examForm] = await Promise.all([
    ...
    listCertDrillAdminScenariosServer(certificationId),
  ...
  return <ExamFormEditor certification={certification} categories={categories} questions={questions} scenarios={scenarios} examForm={examForm} />;
}
